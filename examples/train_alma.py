...
def main(cfg):
    ...
    for epoch in tqdm.trange(cfg.epochs, leave=False):
        ...
        if (epoch % cfg.eval_on_env_freq == 0):
            ...
            fig_1 = train_env.plot_different_state(plt_indicies = [6, 7, 8, 30, 31, 42], plt_time = 2000, separate_env=True)
            wandb.log({"eval_on_mockenv": wandb.Image(fig_1)})
            plt.close(fig_1)
            fig_2 = train_env.plot_different_state(plt_indicies = [30, 31, 42], plt_time = 2000, separate_env=False)
